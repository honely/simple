...
class AdmininfoController extends Controller {
    ...
    public function addadmin(){
		...
		if (IS_POST){
			...
			if($flag){
				...
				$this->success('添加管理员成功！',U('admininfo/index'),3);
			}else{
				$this->error('添加管理员失败！',U('admininfo/index'),3);
			}
		}else{
    ...
    public function editadmin(){
		...
		if (IS_POST){
			...
			$this->success('编辑管理员成功！',U('admininfo/index'),3);
    ...
    public function pswadmin(){
		...
		if (IS_POST){
			...
			if($flag){
				...
				$this->success('密码修改成功！',U('admininfo/index'),3);
			}else{
				$this->error('密码修改失败！',U('admininfo/index'),3);
			}
		}else{
	...
	public function isable(){
		...
		$this->success('管理员设置成功！',U('admininfo/index'),3);
	}
	//修改账号密码
	public function resetpswadmin(){
		...
		if (IS_POST){
			$admin_id=I("get.admin_id");
			$info=I("info");
			$info_['password']=MD5($info['password']);
			M("admininfo")->where("admin_id=".$admin_id)->save($info_);
			$this->success('密码修改成功！',U('admininfo/resetpswadmin'),3);
		}else{	
			$admininfo=M("admininfo")->where("admin_id=".$admin_id)->find();
			$this->assign('admininfo',$admininfo);
			$this->display();
	...
	public function repassword(){
		...
		M('admininfo')->where('admin_id='.$admin_id)->save($info);
		$this->success('密码重置成功！',U('admininfo/index'),3);
	}
	...
	public function deladmin(){
		...
		$this->success('管理员删除成功！',U('admininfo/index'),3);
	}
