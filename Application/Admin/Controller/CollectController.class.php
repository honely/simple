...
class CollectController extends Controller{
    ...
    public function index(){
        ...
        $where.= " and ec_collect.collect_addtime>=".strtotime($starttime."00:00:00")." ";
        $where.= " and ec_collect.collect_addtime<=".strtotime($endtime."23:59:59")." ";

        //查询满足要求的总记录数
        $count = M("collect")
	       	->join(" left join ec_user as a ON a.user_id = ec_collect.user_id")
            ->join(" left join ec_course as b ON b.course_id = ec_collect.course_id")
            ->join(" left join ec_lesson as c ON c.lesson_id = ec_collect.lesson_id")
            ->where($where)
            ->count();
       ...
       $Pageshow = $Page->adminshow();
        ...
        $listinfo = M("collect")
            ...
            ->join(" left join ec_course as b ON b.course_id = ec_collect.course_id")
			->join(" left join ec_lesson as c ON c.lesson_id = ec_collect.lesson_id")
			->field("ec_collect.*,a.nickname as nickname ,a.phone as phone,c.lesson_name as lesson_name,b.course_name as course_name,b.course_speaker as course_speaker")
			->where($where)
    ...
    public function delcollect(){
        ...
        $this->success("删除成功",U("Collect/index"));
    }
}