...
class Comment2aloneController extends Controller
{
    ...
    public function index(){
        $where="1 and ec_comment2alone.comment2_flag=1 ";
        $info = I("info");
        if($info){
            @extract($info);
            //按姓名
        ...
        $comment = M("Comment2alone")
            ...
            ->join(" left join ec_user ON ec_user.user_id = ec_comment2alone.user_id")
            ->join(" left join ec_commentalone  ON ec_commentalone.comment_id = ec_comment2alone.comment_id")
            ->field("ec_comment2alone.*,ec_user.nickname as nickname,ec_commentalone.comment as comment")
    ...
    public function shenhe(){
        ...
        $flag=M("comment2alone")->where("comment2_id=".$comment2_id)->save(array('comment2_flag'=>2));
        if($flag){
            $this->success('审核评论的评论成功！',U('comment2alone/index'),3);
        }else{
            $this->error('审核评论的评论失败！',U('comment2alone/index'),3);
        }
    ...
    public function delcomment2(){
        ...
        $this->success('删除成功！',U('comment2alone/index'),3);
    }
