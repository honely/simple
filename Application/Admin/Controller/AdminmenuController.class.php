...
class AdminmenuController extends Controller {
    ...
    public function addmenuinfo(){
		...
		if (IS_POST){
			...
			if($flag){
				...
				$this->success('添加菜单成功！',U('adminmenu/index',array("menu_id"=>$info['menu_fatherid'])),3);
			}else{
				$this->error('添加菜单失败！',U('adminmenu/index',array("menu_id"=>$info['menu_fatherid'])),3);
			}
		}else{
    ...
    public function editmenuinfo(){
		...
		if (IS_POST){
			...
			$this->success('编辑菜单成功！',U('adminmenu/index',array("menu_id"=>$info['menu_fatherid'])),3);
	...
	public function delmenuinfo(){
		...
		$this->success('菜单删除成功！',U('adminmenu/index'),3);
	}
