	...
	<body>
		<div class="right_min">
			...
			<div class="min">
				...
				<div class="query">
					<form  name="seachform" id="seachform"  action="<?php echo U('moneyinfo/index');?>"  method="post" class="form"  onsubmit="return checkkwd()">
						<div class="form_input mg_trb">
							<label class="text">快速检索</label>
							<input type="text" name="info[keyword]" id="keyword" value="<?php echo ($keyword); ?>"  placeholder="请输入用户姓名或手机号"  maxlength="11" style="width: 150px;" />
						</div>
						<div class="select mg_trb">		
							<span class="text">变动类型</span>
							<select name="info[money_type]">
								<option value="">--请选择--</option>
								<?php if(is_array($moneytype)): $i = 0; $__LIST__ = $moneytype;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if($key == $money_type): ?>selected<?php endif; ?>><?php echo ($data); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
							</select>
						</div>		
						...
						<div class="button mg_trb">
							...
							<input type="button" value="重置" class="but chongzhi" onclick="location.href='<?php echo U('moneyinfo/index');?>'"/>
						</div>
					...
					<table class="table" border="" cellspacing="" cellpadding="">
						...
						<tbody>
							<?php if(is_array($listinfo)): $i = 0; $__LIST__ = $listinfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
								<td><?php echo ($v["money_id"]); ?></td>
								<td><?php echo ($v["nickname"]); ?></td>
								<td><?php echo ($v["phone"]); ?></td>
								<td><?php echo ($v["money_money"]); ?></td>
								<td><?php echo ($moneytype[$v[money_type]]); ?></td>
								<td><?php echo (date("Y-m-d H:i:s",$v["money_addtime"])); ?></td>
								<td><?php echo ($v["money_remarks"]); ?></td>
								<td>
								<a href="<?php echo U('moneyinfo/delmoneyinfo',array('money_id'=>$v['money_id']));?>" onclick="{if(confirm('确认删除?')){return true;}return false;}" class="but chongzhi">
