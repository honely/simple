...
<html>
    ...
    <head>
        ...
        <script src="/Public/home/js/jquery.pin.js" type="text/javascript" charset="utf-8"></script>
    ...
    <body>
		...
		<?php if($course['classify_id']!=2):?><!--免费的都不显示购买课程，但是可能会显示入学-->
			<?php if(!$user_auth):?>
			<div class="tuiguang">
				<table border="0">
					<tr>
						<td>
							<p class="text">简异思维   全年至少100节视频大课等着你</p>
						</td>
						<td onclick="location.href='<?php echo U('course/xuanzezhifu',array('course_id'=>$course['course_id']));?>'">
							<span class="but">订阅专题</span>
						</td>
					</tr>
				</table>
			</div>
            ...
            <div class="center">
                            ...
                            <dl class="twlb c">
                                ...
                                <dd>
                                    <div class="top">
                                        <a href="<?= U('course/video',array('course_id'=>$v['course_id']))?>">
                                            <h4>
                                                <strong><?= $v['course_name']?></strong>
                                            </h4>
                                            <p><?= $v['course_remarks']?></p>
                                        </a>
                                    </div>
                                    <div class="ct">
                                        <div class="left">
					...
					<div id="jianjie">  
						<div class="jianjie_box">
							<div class="head_tatle">
								...
								<h4><?php echo ($current_lesson["lesson_name"]); ?></h4>
							</div>
							<div class="jianjie_ct"><?php echo ($current_lesson["lesson_content"]); ?></div>
						</div>
					</div>
                </div>
                <div class="center_box">
                    <div class="hudong_box">
                        ...
                        <div id='comments'>
                        <?php foreach($course['comments'] AS $k=>$v):?>
                        <dl class="pinglun">
                            ...
                            <dd>
                                ...
                                <div class="huifu" id='huifu<?=$v['comment_id']?>'>
                                    <?php foreach($v['comments2'] AS $k2=>$v2):?>
                                    <dl class="pinglun">
...
<script>
                            ...
                            player.on("play", function(){
                               ...
                               // $.get("<?=U('course/add_play_number',array('course_id'=>$course['course_id']))?>");
                                 //每隔5秒 提交一次观看信息
                                sh =setInterval(function(){$.get("<?=U('course/learntime',array('course_id'=>$course['course_id'],'lesson_id'=>$current_lesson_id))?>")},5000);
                            });
                            ...
                            function loadurl(obj) { 
                                ...
                                //alert(lesson_id); 
                                window.location.href="<?= U('course/video',array('course_id'=>$course[course_id]))?>&lesson_id="+lesson_id;
                            }
                            function pinglu(a) { //评论弹出  
                            $("#pinglun_form").attr("to",a);
                            if(a == 0){ //对专题评论 
                                    $("#pinglun_form").attr("action","<?= U('course/course_comment',array('course_id'=>$course['course_id'],'lesson_id'=>$current_lesson_id))?>");
                                }else{ //对评论回复
                            ...
                            function shoucang(obj) { 
                                if($(obj).hasClass('link')){ //取消收藏 
                                    $.get("<?=U('course/quxiaoshoucang',array('course_id'=>$course['course_id'],'lesson_id'=>$current_lesson_id))?>");
                                    ...
                                    $.get("<?=U('course/shoucang',array('course_id'=>$course['course_id'],'lesson_id'=>$current_lesson_id))?>");
                                    $(obj).addClass('link'); 
                                      layer.open({
                                          content: '收藏成功',
                                          skin: 'msg',
                           ...
                           $.ajax({
                            ...
                            //跳转到音频播放
                            function tiaozhuan(){ 
                               location.href="<?php echo U('course/audio',array('course_id'=>$course['course_id'],'lesson_id'=>$current_lesson_id));?>";
                    ...
                    function getmore() {
                        ...
                        //alert(page); 
                        $.getJSON("<?php echo U('course/gvideo',array('course_id'=>$course['course_id'],'lesson_id'=>$current_lesson_id));?>&p=" + page, function (data) { 
                            if (data.length > 0) {
                                var html = ""; 
